...
class Lets(LetsCore):
    ...
    def fuzzy_find(lets: "Lets", text: str, options: List[str], unique=False, starts=False, case_sensitive=False, require_match=False, no_match_error=None) -> Optional[List[str]]:
        ...
        Args:
            ...
            error_text: The text to print when the text is not found. When a string is provided, the no_match_error is printed and
        ...
        regex = re.compile(("^" if starts else "") + re.escape(text), 0 if case_sensitive else re.IGNORECASE)
        results = [s for s in options if regex.search(s)]
        if require_match and (not results or (unique and len(results) != 1)):
            raise LetsExcept(f"{no_match_error}: '{text}'")
        return results

    ####################################################
    ######    Functions to execute commands       ######
    ####################################################
    def execute(lets: "Lets", command: str, show_output: bool=True):
        ...
        lets.verbose(f"Executing command: {command}")
