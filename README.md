...
### Fuzzy
Plugin developers can enable fuzzy argument matching. Useful when users don’t remember exact names.
...
### Devcontainers
Devcontainers provide an easy way to set up reproducible, version‑controlled build environments. Lets includes native support for devcontainers, so users no longer need to configure their local setup manually. Lets automatically detects when it is running inside a container and executes the command natively in that context.
...
A plugin can easily add devcontainer support thanks to the built‑in integration provided by Lets.

```python
@verb("build")
def build(lets: "Lets", _verb: str, args: List[str]) -> int:
    devcontainer = DevContainer(str(ROOT_DIR), (str(ROOT_DIR), "/workspaces/myproject"))
    lets.execute("ls -l", devcontainer)
